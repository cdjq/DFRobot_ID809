...
uint8_t DFRobot_ID809::collectionFingerprint(uint16_t timeout){  //采集指纹
    uint16_t i = 0;
    uint8_t ret;
    if(_number > 2){
...
const DFRobot_ID809::sErrorDescription_t DFRobot_ID809::errorDescriptionsTable[]={
  {eErrorSuccess, "指令处理成功"},
  {eErrorFail, "指令处理失败"},
  {eErrorVerify, "1:1对比失败"},
  {eErrorIdentify, "与全部指纹对比失败"},
  {eErrorTmplEmpty, "指定ID中不存在指纹"},
  {eErrorTmplNotEmpty, "指定ID中存在指纹"},
  {eErrorAllTmplEmpty, "模块未注册指纹"},
  {eErrorEmptyIDNoexist, "不存在可注册ID"},
  {eErrorBrokenIDNoexist, "不存在损坏的指纹"},
  {eErrorInvalidTmplData, "指定的指纹数据无效"},
  {eErrorDuplicationID, "该指纹已注册"},
  {eErrorBadQuality, "指纹图像质量不好"},
  {eErrorMergeFail, "指纹合成失败"},
  {eErrorNotAuthorized, "没有进行通讯密码确认"},
  {eErrorMemory, "外部Flash烧写出错"},
  {eErrorInvalidTmplNo, "指定ID无效"},
  {eErrorInvalidParam, "使用了不正确的参数"},
  {eErrorTimeOut, "采集超时"},
  {eErrorGenCount, "指纹合成个数无效"},
  {eErrorInvalidBufferID, "Buffer ID值不正确"},
  {eErrorFPNotDetected, "采集器上没有指纹输入"},
  {eErrorFPCancel, "指令被取消"},
  {eErrorRecvLength, "数据长度错误"},
  {eErrorRecvCks, "数据校验码错误"},
  {eErrorGatherOut, "采集次数超过上限"},
  {eErrorRecvTimeout,"读取数据超时"}
};
